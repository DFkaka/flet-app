...
def data_row(leading_text: str, title: str, subtitle: str, trailing_text: str = '',
             on_click=None, avatar_color=ft.colors.BLUE_700) -> ft.Container:
    """通用数据行组件"""
    return ft.Container(
        content=ft.ListTile(
            ...
            trailing=ft.Text(trailing_text, size=13, color=ft.colors.GREY_700)
...
def empty_state(message: str = '暂无数据') -> ft.Container:
    ...
    return ft.Container(
        content=ft.Column([
            ...
            ft.Text(message, color=ft.colors.GREY_500),
...
def status_badge(status: str) -> ft.Container:
    """状态标签"""
    colors = {
        'pending': (ft.colors.ORANGE_100, ft.colors.ORANGE_900, '待处理'),
        'received': (ft.colors.GREEN_100, ft.colors.GREEN_900, '已收货'),
        'shipped': (ft.colors.GREEN_100, ft.colors.GREEN_900, '已发货'),
        'cancelled': (ft.colors.RED_100, ft.colors.RED_900, '已取消'),
        'draft': (ft.colors.GREY_100, ft.colors.GREY_900, '草稿'),
    }
    bg, fg, label = colors.get(status, (ft.colors.GREY_100, ft.colors.GREY_900, status))
    return ft.Container(
...
def detail_row(label: str, value: str) -> ft.Row:
    ...
    return ft.Row([
        ft.Text(label + '：', size=14, weight=ft.FontWeight.BOLD,
               color=ft.colors.GREY_700, width=80),
